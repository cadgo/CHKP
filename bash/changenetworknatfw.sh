...
nfw="VS_VoIP"
hstfile="networks.file"
hostdb="network_chkp.db"
totalhosts=`cat $hstfile | jq '.total'`
...
do
        ...
        if [[ "$bb" = "Cluster_VoIP" ]]; then
                if [[ "$nattype" = "static" ]]; then
                        ...
                        echo "Chaging Fw Nat to network " $xx " en modo static" >> $logfile
                        mgmt_cli -r true -d 150.100.218.18 set network name $xx nat-settings.auto-rule $auto nat-settings.ip-address $ipv4 nat-settings.method "static" nat-settings.install-on $nfw
                else
                        ipv4=`cat $hstfile | jq ".objects[] | select(.name==$xx)" | jq '."nat-settings"."ipv4-address"' | sed -e 's/^"//' -e 's/"$//'`
                        auto=`cat $hstfile | jq ".objects[] | select(.name==$xx)" | jq '."nat-settings"."auto-rule"' | sed -e 's/^"//' -e 's/"$//'`
                        echo "Changing Fw NAt to network " $xx " en modo hide" >> $logfile
                        mgmt_cli -r true -d 150.100.218.18 set network name $xx nat-settings.auto-rule $auto nat-settings.ip-address $ipv4 nat-settings.method "hide" nat-settings.install-on $nfw
                fi
        fi
done
